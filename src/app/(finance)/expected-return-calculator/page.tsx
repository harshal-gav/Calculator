...
export default function ExpectedReturnCalculator() {
    ...
    const calculate = () => {
        ...
        if (Math.abs(totalProb - 100) > 0.1) {
            setError(`Warning: Probabilities add up to ${totalProb.toFixed(1)}%, not 100%. Results are mathematically valid based on what you entered, but usually they should equal 100%.`);
        }
        ...
        <div className="max-w-4xl mx-auto p-4 md:p-8 bg-zinc-50 rounded-2xl shadow-xl border border-zinc-200">
            ...
            <div className="bg-white p-6 md:p-10 rounded-2xl shadow-sm border border-zinc-200 mb-8 max-w-2xl mx-auto">
                <div className="flex justify-between items-center mb-6">
                    <h3 className="font-bold text-zinc-700 uppercase tracking-wide">Investment Scenarios</h3>
                    ...
                    {scenarios.map((scenario, idx) => (
                        <div key={idx} className="flex gap-4 items-center">
                            <div className="flex-1">
                                <label className="block text-[10px] font-bold text-zinc-500 mb-1 uppercase tracking-widest pl-1">Probability (%)</label>
                                <input
                                    type="number" step="any" min="0" max="100" value={scenario.prob} onChange={(e) => updateScenario(idx, 'prob', e.target.value)}
                                    className="w-full rounded-xl border-zinc-300 shadow-sm p-3 border focus:border-emerald-500 font-bold font-mono"
                                    placeholder="e.g. 50"
                                />
                            </div>
                            <div className="flex-1">
                                <label className="block text-[10px] font-bold text-zinc-500 mb-1 uppercase tracking-widest pl-1">Expected Return (%)</label>
                                <input
                                    type="number" step="any" value={scenario.return} onChange={(e) => updateScenario(idx, 'return', e.target.value)}
                                    className="w-full rounded-xl border-zinc-300 shadow-sm p-3 border focus:border-emerald-500 font-bold font-mono"
                                    placeholder="e.g. 8.5"
                                />
                            </div>
            ...
            {result !== null && (
                <div className="bg-slate-900 rounded-2xl p-6 md:p-10 shadow-2xl relative overflow-hidden flex flex-col items-center">
                    ...
                    <h2 className="text-emerald-400 font-bold uppercase tracking-widest text-xs mb-8 z-10 text-center">Probability-Weighted Return</h2>
                    ...
                    <div className="z-10 relative mb-8 w-full max-w-sm">
                        <div className="p-8 rounded-3xl border-4 bg-emerald-900/40 border-emerald-500/30 shadow-inner flex flex-col items-center justify-center">
                            <span className="text-emerald-300 text-[10px] font-bold uppercase tracking-widest mb-3 block border-b border-emerald-500/50 pb-2 w-full text-center">Expected Rate</span>
                            <div className="font-mono font-black text-5xl md:text-6xl text-white break-all tracking-tight drop-shadow-lg p-2">
                                {result.expectedReturn.toLocaleString('en-US', { maximumFractionDigits: 3 })}%
                            </div>
                        </div>
                    </div>

                    <div className="bg-black/30 px-6 py-3 rounded-xl border border-white/5 text-center flex flex-col items-center z-10 text-xs">
                        <span className="text-white/40 uppercase font-bold tracking-widest">Calculated across {result.totalProb.toFixed(1)}% total probability matrix</span>
                    </div>
