    ...
    <div className="max-w-4xl mx-auto p-4 md:p-8 bg-zinc-50 rounded-2xl shadow-xl border border-zinc-200">
      <div className="text-center mb-10">
        ...
        <p className="text-sky-700 text-lg max-w-2xl mx-auto">
          Calculate your Loan-to-Value (LTV) ratio to instantly determine
          mortgage risk and Private Mortgage Insurance (PMI) requirements.
        </p>
      </div>
      ...
      <div className="bg-white p-6 md:p-8 rounded-2xl shadow-sm border border-zinc-200 mb-8 max-w-2xl mx-auto">
        <div className="space-y-6 mb-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 relative">
            <div className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 bg-zinc-100 text-zinc-400 font-bold text-[10px] px-2 py-1 rounded-full uppercase tracking-widest hidden md:block z-10 border border-zinc-200">
              OR
            </div>

            <div>
              <label className="block text-xs font-bold text-sky-700 mb-2 uppercase tracking-wide">
                Total Loan Amount
              </label>
              <div className="relative">
                <span className="absolute left-4 top-3 text-zinc-400 font-bold">
            ...
            <div>
              <label className="block text-xs font-bold text-teal-700 mb-2 uppercase tracking-wide">
                Down Payment
              </label>
              <div className="relative">
                <span className="absolute left-4 top-3 text-zinc-400 font-bold">
      ...
      {result !== null && (
        <div
          className={`p-6 md:p-10 shadow-2xl relative overflow-hidden flex flex-col items-center rounded-2xl border ${result.needsPMI ? "bg-rose-950 border-rose-800" : "bg-teal-950 border-teal-800"}`}
        >
          <div
            className={`absolute top-0 right-0 w-64 h-64 rounded-full mix-blend-screen filter blur-[80px] opacity-20 pointer-events-none ${result.needsPMI ? "bg-rose-500" : "bg-teal-500"}`}
          ></div>

          <h2 className="text-white/60 font-bold uppercase tracking-widest text-xs mb-8 z-10 text-center">
            Calculated Loan-to-Value Ratio
          </h2>

          <div className="z-10 relative mb-8 w-full max-w-sm">
            <div
              className={`p-8 rounded-full aspect-square border-4 flex flex-col items-center justify-center shadow-inner ${result.needsPMI ? "bg-rose-900/40 border-rose-500/50 shadow-rose-900/50" : "bg-teal-900/40 border-teal-500/50 shadow-teal-900/50"}`}
            >
              <div
                className={`font-mono font-black text-6xl break-all drop-shadow-lg ${result.needsPMI ? "text-rose-400" : "text-teal-400"}`}
              >
                {result.ltv.toFixed(2)}
                <span className="text-3xl">%</span>
              </div>
              <span className="text-white/50 text-[10px] uppercase tracking-widest mt-2 font-bold">
                LTV Ratio
              </span>
            </div>
          </div>
          ...
          <div className="w-full max-w-2xl z-10 bg-black/30 p-6 rounded-2xl border border-white/10 mb-6">
            ...
            <div className="flex justify-between items-center mb-4 pb-4 border-b border-white/10">
              <span className="text-sky-300 text-sm font-bold tracking-wide">
                Loan Amount
              </span>
              <span className="font-mono text-sky-200 text-lg font-bold">
                ${result.loanAmount.toLocaleString()}
              </span>
            </div>
            <div className="flex justify-between items-center">
              <span className="text-teal-300 text-sm font-bold tracking-wide">
                Down Payment
              </span>
              <div className="text-right">
                <span className="font-mono text-teal-200 text-lg font-bold block">
                  ${result.downPayment.toLocaleString()}
                </span>
                <span className="text-teal-500/80 text-[10px] uppercase tracking-widest font-bold">
          ...
          <div
            className={`z-10 w-full max-w-2xl p-4 rounded-xl border flex items-start gap-4 ${result.needsPMI ? "bg-rose-900/40 border-rose-500/30" : "bg-teal-900/40 border-teal-500/30"}`}
          >
            <div className="text-3xl">{result.needsPMI ? "⚠️" : "✅"}</div>
            <div>
              <h3
                className={`font-bold uppercase tracking-widest text-xs mb-1 ${result.needsPMI ? "text-rose-300" : "text-teal-300"}`}
              >
      ...
      <CalculatorSEO
        ...
        whatIsIt={
          <p>
            The <strong>Loan-to-Value (LTV) Calculator</strong> is a primary
            real estate finance tool used by lenders to assess the risk of a
            mortgage. It compares your total loan amount to the appraised value
            of the property, outputting a clear percentage.
          </p>
        }
        formula={
          <>
        ...
        example={
          <>
            <p>
              If you buy a house for $300,000 and put down $60,000 in cash, you
              need a $240,000 loan:
            </p>
        ...
        useCases={
          <ul className="list-disc pl-6 space-y-4">
            <li>
              <strong>Avoiding PMI (Private Mortgage Insurance):</strong> If
              your LTV is strictly 80% or lower (meaning your down payment was
              at least 20%), you legally avoid paying expensive monthly PMI fees
              on conventional loans.
            </li>
            <li>
              <strong>Home Refinancing:</strong> To secure the best interest
              rates when refinancing, lenders want to see an LTV under 80% based
              on your home's current market value, not what you originally paid
              for it.
            </li>
            <li>
              <strong>HELOC Approvals:</strong> Home Equity Lines of Credit
              usually require your combined LTV (your first mortgage + the new
              line of credit) to remain under 85% of your home's total value.
            </li>
          </ul>
        }
        faqs={[
          {
            ...
            answer:
              "Private Mortgage Insurance (PMI) is an insurance policy you pay for, but it protects the lender, not you. If you default on your mortgage and your LTV was high, the lender uses the PMI to recoup their losses.",
          },
          {
