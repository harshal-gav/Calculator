    ...
    <div className="max-w-7xl mx-auto p-4 md:p-8 bg-zinc-50 rounded-2xl shadow-xl border border-zinc-200">
      <div className="text-center mb-10">
        ...
        <p className="text-indigo-700 text-lg max-w-2xl mx-auto">
          Compare two text snippets side-by-side to find line differences
          instantly.
        </p>
      </div>
      ...
      {diffResult !== null && (
        <div className="bg-slate-900 rounded-2xl p-4 md:p-8 shadow-2xl relative overflow-hidden flex flex-col">
          <h2 className="text-indigo-400 font-bold uppercase tracking-widest text-xs mb-6 z-10 text-center">
            Diff Viewer (Line-by-Line Match)
          </h2>
            ...
            <div className="bg-zinc-800 flex flex-col overflow-x-auto min-h-[200px]">
              {diffResult.originalLines.map((line, idx) => {
                ...
                return (
                  <div
                    key={idx}
                    className={`flex px-2 py-0.5 whitespace-pre ${isDiff ? "bg-red-900/30 text-red-200" : "text-zinc-400"}`}
                  >
                    <span className="w-8 text-right pr-2 select-none opacity-50 border-r border-zinc-700 mr-2 shrink-0">
                      {idx + 1}
                    </span>
                    <span>{line || " "}</span>
                  </div>
            ...
            <div className="bg-zinc-800 flex flex-col overflow-x-auto min-h-[200px]">
              {diffResult.modifiedLines.map((line, idx) => {
                ...
                return (
                  <div
                    key={idx}
                    className={`flex px-2 py-0.5 whitespace-pre ${isDiff ? "bg-emerald-900/40 text-emerald-300" : "text-zinc-400"}`}
                  >
                    <span className="w-8 text-right pr-2 select-none opacity-50 border-r border-zinc-700 mr-2 shrink-0">
                      {idx + 1}
                    </span>
          ...
          <div className="text-center mt-4">
            <span className="text-xs text-white/40 uppercase tracking-widest">
              Only exact line-by-line index match is performed
            </span>
          </div>
        </div>
      )}
      ...
      <div className="mt-8">
        <CalculatorSEO
          ...
          whatIsIt={
            <>
              <p>
                The <strong>Diff Checker</strong> performs a side-by-side
                comparison of two blocks of text or code, instantly highlighting
                exactly which lines have been added, modified, or deleted.
              </p>
              <p>
                When dealing with thousands of lines of code or complex legal
                documents, finding a single changed word manually is practically
                impossible. Diff (difference) checking automates this entirely,
                acting as the foundation for modern version control systems like
                Git.
              </p>
            </>
          }
          ...
          example={
            <>
              <p>
                You have a configuration file causing a server crash. You
                compare the current broken version against last week's working
                backup.
              </p>
              <ul className="list-disc pl-6 space-y-2 mt-4 text-gray-700">
                <li>
                  <strong>Original (Line 42):</strong>{" "}
                  <code>timeout_seconds = 600</code>
                </li>
                <li>
                  <strong>Modified (Line 42):</strong>{" "}
                  <code>timeout_seconds = 60</code>
                </li>
                <li>
                  <strong>The Result:</strong> The Diff Checker instantly flags
                  Line 42 in stark red/green, revealing someone accidentally
                  deleted a single '0', causing the server to timeout 10x
                  faster.
                </li>
              </ul>
            </>
          ...
          useCases={
            <ul className="list-disc pl-6 space-y-4 text-gray-700">
              <li>
                <strong>Software Development:</strong> Before merging new code
                into a master project, developers use diffs (like GitHub Pull
                Requests) to review the exact lines of code a teammate altered,
                ensuring no malicious or buggy code is introduced.
              </li>
              <li>
                <strong>Legal & Contracts:</strong> Lawyers compare generic
                contract templates against modified versions returned by the
                opposing party to ensure no sneaky, unapproved clauses were
                inserted into the middle of a 50-page PDF.
              </li>
              <li>
                <strong>Content Auditing:</strong> Editors compare article
                drafts to see exactly which sentences a freelance writer updated
