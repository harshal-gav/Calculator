        ...
        <div className="max-w-4xl mx-auto p-4 md:p-8 bg-zinc-50 rounded-2xl shadow-xl border border-zinc-200">
            <div className="text-center mb-10">
                ...
                <p className="text-rose-700 text-lg max-w-2xl mx-auto">
                    Calculate exactly how far, how high, and how long a projectile will travel given initial velocity and launch angle.
                </p>
            </div>
            ...
            <div className="bg-white p-6 md:p-8 rounded-2xl shadow-sm border border-zinc-200 mb-8 max-w-3xl mx-auto">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                    ...
                    <div>
                        <label className="block text-xs font-bold text-zinc-500 mb-1 uppercase tracking-widest">Initial Velocity (v₀)</label>
                        <div className="flex">
                            <input
                                type="number" step="any" min="0" value={velocity} onChange={(e) => setVelocity(e.target.value)}
                                className="w-full rounded-l-xl border-zinc-300 shadow-sm p-3 border focus:border-rose-500 font-bold bg-zinc-50 font-mono"
                            />
                            <div className="bg-zinc-100 border border-l-0 border-zinc-300 rounded-r-xl px-4 flex items-center font-bold text-zinc-500 text-sm">m/s</div>
                        </div>
                    </div>

                    <div>
                        <label className="block text-xs font-bold text-zinc-500 mb-1 uppercase tracking-widest">Launch Angle (θ)</label>
                        <div className="flex">
                            <input
                                type="number" step="any" min="0" max="90" value={angle} onChange={(e) => setAngle(e.target.value)}
                                className="w-full rounded-l-xl border-zinc-300 shadow-sm p-3 border focus:border-rose-500 font-bold bg-zinc-50 font-mono"
                            />
                            <div className="bg-zinc-100 border border-l-0 border-zinc-300 rounded-r-xl px-4 flex items-center font-bold text-zinc-500 text-sm">Deg</div>
                        </div>
                    </div>

                    <div>
                        <label className="block text-xs font-bold text-zinc-500 mb-1 uppercase tracking-widest">Initial Height (h₀)</label>
                        <div className="flex">
                            <input
                                type="number" step="any" min="0" value={height} onChange={(e) => setHeight(e.target.value)}
                                className="w-full rounded-l-xl border-zinc-300 shadow-sm p-3 border focus:border-rose-500 font-bold bg-zinc-50 font-mono"
                            />
                            <div className="bg-zinc-100 border border-l-0 border-zinc-300 rounded-r-xl px-4 flex items-center font-bold text-zinc-500 text-sm">m</div>
                        </div>
                    </div>

                    <div>
                        <label className="block text-xs font-bold text-zinc-500 mb-1 uppercase tracking-widest">Gravity (g)</label>
                        <div className="flex">
                            <input
                                type="number" step="any" min="0" value={gravity} onChange={(e) => setGravity(e.target.value)}
                                className="w-full rounded-l-xl border-zinc-300 shadow-sm p-3 border focus:border-rose-500 font-bold bg-zinc-50 font-mono"
            ...
            {result !== null && (
                <div className="bg-slate-900 rounded-2xl p-6 md:p-10 shadow-2xl relative overflow-hidden flex flex-col items-center">
                    ...
                    <h2 className="text-white/60 font-bold uppercase tracking-widest text-xs mb-8 z-10 text-center">Kinematic Results</h2>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6 w-full max-w-4xl z-10 relative items-center mb-10">
                        <div className="bg-black/40 p-6 rounded-2xl border border-rose-500/30 flex flex-col items-center justify-center text-center shadow-inner h-full">
                            <span className="text-rose-400 text-[10px] font-bold uppercase tracking-widest mb-3">Max Height (h_max)</span>
                            <div className="font-mono text-white tracking-tight font-black text-4xl break-all">
                                {result.maxHeight.toLocaleString('en-US', { maximumFractionDigits: 2 })}
                        ...
                        <div className="bg-gradient-to-tr from-rose-900 to-pink-900 p-6 rounded-2xl border border-rose-400/30 flex flex-col items-center justify-center text-center shadow-xl h-full transform md:scale-110">
                            <span className="text-rose-200 text-[10px] font-bold uppercase tracking-widest mb-3">Total Range (R)</span>
                            <div className="font-mono text-white tracking-tight font-black text-5xl break-all drop-shadow-lg">
                                {result.range.toLocaleString('en-US', { maximumFractionDigits: 2 })}
                                <span className="text-rose-300 text-xl ml-1">m</span>
                            </div>
                        </div>

                        <div className="bg-black/40 p-6 rounded-2xl border border-rose-500/30 flex flex-col items-center justify-center text-center shadow-inner h-full">
                            <span className="text-rose-400 text-[10px] font-bold uppercase tracking-widest mb-3">Flight Time (t)</span>
                            <div className="font-mono text-white tracking-tight font-black text-4xl break-all">
                                {result.flightTime.toLocaleString('en-US', { maximumFractionDigits: 3 })}
                    ...
                    <div className="z-10 bg-black/50 p-6 rounded-xl border border-white/5 w-full max-w-4xl max-h-48 overflow-y-auto">
                        <span className="text-zinc-500 text-xs font-bold uppercase tracking-widest mb-4 block sticky top-0 bg-black/90 pb-2">Trajectory Data Points (x, y)</span>
                        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4 font-mono text-sm">
                            {result.trajectory.map((point, i) => (
                                <div key={i} className="bg-white/5 border border-white/10 p-2 rounded text-center">
                                    <div className="text-rose-400 text-[10px] uppercase mb-1">t = {(i * result.flightTime / 20).toFixed(2)}s</div>
