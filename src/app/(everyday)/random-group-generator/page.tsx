        ...
        <div className="max-w-4xl mx-auto p-4 md:p-8 bg-zinc-50 rounded-2xl shadow-xl border border-zinc-200">
            ...
            <div className="bg-white p-6 md:p-8 rounded-2xl shadow-sm border border-zinc-200 mb-8 max-w-3xl mx-auto">
                <div className="space-y-6 mb-8">
                    <div>
                        <label className="block text-sm font-bold text-zinc-700 mb-2 uppercase tracking-wide">Names or Items</label>
                        <p className="text-xs text-zinc-500 mb-3 font-medium">Separate by commas or put each item on a new line.</p>
                        <textarea
                            rows={5}
                            value={namesInput}
                    ...
                    <div>
                        <label className="block text-sm font-bold text-zinc-700 mb-2 uppercase tracking-wide">Number of Groups</label>
                        <input
                            type="number" step="1" min="1" value={numGroups} onChange={(e) => setNumGroups(e.target.value)}
            ...
            {result !== null && (
                <div className="bg-slate-900 rounded-2xl p-6 md:p-10 shadow-2xl relative overflow-hidden flex flex-col items-center">
                    ...
                    <h2 className="text-purple-400 font-bold uppercase tracking-widest text-xs mb-8 z-10 text-center">Assigned Groups</h2>

                    <div className="w-full max-w-4xl z-10 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        {result.map((group, idx) => (
                            <div key={idx} className="bg-black/40 border-t-4 border-purple-500 rounded-xl border-x border-b border-white/5 overflow-hidden flex flex-col h-full shadow-lg">
                                <div className="bg-black/20 p-3 text-center border-b border-white/5">
                                    <span className="text-purple-300 font-black uppercase tracking-widest text-sm">Group {idx + 1}</span>
                                    <span className="text-white/30 text-[10px] block font-bold uppercase mt-1">{group.length} members</span>
                                </div>
                                <div className="p-4 flex-1">
                                    <ul className="space-y-2">
                                        {group.map((item, itemIdx) => (
                                            <li key={itemIdx} className="bg-white/5 rounded-lg px-3 py-2 text-white font-medium flex items-center">
                                                <span className="w-6 h-6 rounded-full bg-purple-500/20 text-purple-300 text-xs flex items-center justify-center mr-3 font-bold border border-purple-500/30">{itemIdx + 1}</span>
