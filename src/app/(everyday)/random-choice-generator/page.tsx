...
export default function RandomChoiceGenerator() {
  ...
  const generate = () => {
    ...
    if (!allowDuplicates && c > items.length) {
      setError(
        `Cannot pick ${c} unique items from a list of ${items.length} items.`,
      );
      setResults([]);
      return;
    }
    ...
    <div className="max-w-4xl mx-auto p-4 md:p-8 bg-zinc-50 rounded-2xl shadow-xl border border-zinc-200">
      <div className="text-center mb-10">
        ...
        <p className="text-violet-700 text-lg max-w-2xl mx-auto">
          Pick a random item, winner, or choice from any list instantly. Let the
      ...
      {results.length > 0 && (
        <div className="bg-violet-950 rounded-2xl p-6 md:p-10 shadow-2xl relative overflow-hidden flex flex-col items-center">
          ...
          <h2 className="text-violet-400 font-bold uppercase tracking-widest text-xs mb-6 z-10">
            Selected {results.length === 1 ? "Choice" : "Choices"}
          </h2>
          ...
          <div className="z-10 w-full flex flex-col gap-4 max-h-[400px] overflow-y-auto">
            {results.map((choice, index) => (
              <div
                key={index}
                className="bg-white/10 p-4 md:p-6 rounded-xl border border-white/20 shadow-inner flex items-center justify-between gap-4 w-full backdrop-blur-sm transition-transform hover:scale-[1.01]"
              >
