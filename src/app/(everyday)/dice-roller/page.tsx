        ...
        <div className="max-w-4xl mx-auto p-4 md:p-8 bg-zinc-50 rounded-2xl shadow-xl border border-zinc-200">
            ...
            <div className="bg-white p-6 md:p-10 rounded-2xl shadow-sm border border-zinc-200 mb-8 max-w-3xl mx-auto">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8 items-end">
                    ...
                    <div>
                        <label className="block text-sm font-bold text-zinc-600 mb-2 uppercase tracking-wide">Number of Dice</label>
                        <div className="flex bg-zinc-50 border border-zinc-200 rounded-xl overflow-hidden shadow-sm">
                            <button onClick={() => setDiceCount(Math.max(1, diceCount - 1))} className="w-12 bg-white font-bold text-xl hover:bg-zinc-100 transition-colors">-</button>
                            <input type="number" min="1" max="100" value={diceCount} onChange={(e) => setDiceCount(parseInt(e.target.value) || 1)} className="flex-1 w-full text-center bg-transparent font-bold text-xl outline-none" />
                            <button onClick={() => setDiceCount(Math.min(100, diceCount + 1))} className="w-12 bg-white font-bold text-xl hover:bg-zinc-100 transition-colors">+</button>
                        </div>
                    </div>

                    <div>
                        <label className="block text-sm font-bold text-zinc-600 mb-2 uppercase tracking-wide">Dice Type</label>
                        <select
                            value={diceType}
                    ...
                    <div>
                        <label className="block text-sm font-bold text-zinc-600 mb-2 uppercase tracking-wide">Modifier (+/-)</label>
                        <input
                            type="number"
                            value={modifier}
                ...
                <div className="flex justify-center mb-6">
                    <span className="font-mono bg-red-50 text-red-900 border border-red-200 px-6 py-2 rounded-full font-bold shadow-sm tracking-widest uppercase">
                        Formula: {diceCount}d{diceType} {modifier > 0 ? `+ ${modifier}` : (modifier < 0 ? `- ${Math.abs(modifier)}` : '')}
                    </span>
                </div>
            ...
            {results !== null && !isRolling && (
                <div className="bg-red-950 rounded-2xl p-6 md:p-10 shadow-2xl relative overflow-hidden flex flex-col items-center animate-[popIn_0.3s_ease-out]">
                    ...
                    <h2 className="text-red-400 font-bold uppercase tracking-widest text-xs mb-6 z-10">Total Result</h2>
                    ...
                    <div className="z-10 w-full max-w-2xl">
                        <h3 className="text-red-400/60 font-bold uppercase tracking-widest text-xs mb-4 text-center border-b border-red-500/20 pb-2">Individual Rolls</h3>
                        <div className="flex flex-wrap justify-center gap-3">
                            {results.rolls.map((roll, i) => (
                                <div key={i} className="bg-white text-red-900 font-black text-xl w-14 h-14 rounded-lg flex items-center justify-center shadow-[inset_0_-4px_0_rgba(0,0,0,0.1)] border-2 border-red-200 relative">
                                    {roll}
