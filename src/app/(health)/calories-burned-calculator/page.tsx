        ...
        <div className="max-w-4xl mx-auto p-4 md:p-8 bg-zinc-50 rounded-2xl shadow-xl border border-zinc-200">
            <div className="text-center mb-10">
                ...
                <p className="text-rose-700 text-lg max-w-2xl mx-auto">
                    Calculate exactly how many calories you burn during various exercises and activities.
                </p>
            </div>
            ...
            <div className="bg-white p-6 md:p-10 rounded-2xl shadow-sm border border-zinc-200 mb-8 max-w-2xl mx-auto">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                    <div>
                        <label className="block text-sm font-bold text-zinc-600 mb-2 uppercase tracking-wide">Your Weight</label>
                        <div className="flex bg-zinc-50 border border-zinc-300 rounded-xl overflow-hidden shadow-sm focus-within:border-rose-500 transition-colors">
                            <input
                                type="number" step="any" min="0" value={weight} onChange={(e) => setWeight(e.target.value)}
                                className="w-full p-4 font-bold bg-transparent text-xl outline-none"
                            />
                            <select value={unit} onChange={(e) => setUnit(e.target.value)} className="bg-zinc-100 font-bold px-4 text-zinc-600 outline-none border-l border-zinc-300">
                    ...
                    <div>
                        <label className="block text-sm font-bold text-zinc-600 mb-2 uppercase tracking-wide">Duration (Minutes)</label>
                        <input
                            type="number" step="any" min="1" value={duration} onChange={(e) => setDuration(e.target.value)}
                ...
                <div className="mb-8">
                    <label className="block text-sm font-bold text-zinc-600 mb-2 uppercase tracking-wide">Activity Type</label>
                    <select
                    ...
                    >
                        {activities.map((act, idx) => (
                            <option key={idx} value={idx}>{act.name} {act.met > 0 ? `(MET: ${act.met})` : ''}</option>
                        ))}
                    </select>
                </div>
                ...
                {activities[selectedActivity].met === 0 && (
                    <div className="mb-8">
                        <label className="block text-sm font-bold text-zinc-600 mb-2 uppercase tracking-wide">Custom MET Value</label>
                        <input
                            type="number" step="any" min="0" value={customMet} onChange={(e) => setCustomMet(e.target.value)}
                            className="w-full rounded-xl border-zinc-300 shadow-sm p-4 border focus:border-rose-500 font-bold bg-zinc-50 text-xl"
                        />
                    </div>
                )}
            ...
            {result !== null && (
                <div className="bg-rose-950 rounded-2xl p-6 md:p-10 shadow-2xl relative overflow-hidden flex flex-col items-center">
                    ...
                    <h2 className="text-rose-400 font-bold uppercase tracking-widest text-xs mb-6 z-10 text-center">Total Calories Expended</h2>
                    ...
                    <div className="bg-black/30 px-12 py-8 rounded-2xl border border-rose-500/30 shadow-inner flex flex-col items-center text-center z-10 w-full max-w-md">
                        ...
                        <span className="text-rose-300 font-bold uppercase tracking-widest mt-4">kcals</span>
                    </div>
