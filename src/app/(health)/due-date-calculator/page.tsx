...
export default function DueDateCalculator() {
    ...
    const calculate = () => {
        ...
        } else if (calcMethod === 'ultrasound') { // Ultrasound gives exact current gestational age, but for UI simplicity let's treat dateInput as estimated due date
            due = new Date(input);
        ...
        const diffTime = Math.abs(today.getTime() - (conception.getTime() - (14 * 24 * 60 * 60 * 1000)));
        ...
        const daysRem = Math.ceil((due.getTime() - today.getTime()) / (1000 * 60 * 60 * 24));
        ...
        <div className="max-w-4xl mx-auto p-4 md:p-8 bg-zinc-50 rounded-2xl shadow-xl border border-zinc-200">
            ...
            <div className="bg-white p-6 md:p-8 rounded-2xl shadow-sm border border-zinc-200 mb-8 max-w-2xl mx-auto">
                <div className="space-y-6 mb-8">
                    ...
                    <div>
                        <label className="block text-sm font-bold text-zinc-700 mb-2 uppercase tracking-wide">Calculation Method</label>
                        <select
                            value={calcMethod}
                    ...
                    {calcMethod === 'lmp' && (
                        <div>
                            <label className="block text-sm font-bold text-zinc-700 mb-2 uppercase tracking-wide">Average Cycle Length (Days)</label>
                            <input
                    ...
                    {calcMethod === 'ivf' && (
                        <div>
                            <label className="block text-sm font-bold text-zinc-700 mb-2 uppercase tracking-wide">Type of Embryo Transfer</label>
            ...
            {result !== null && (
                <div className="bg-gradient-to-br from-pink-900 to-rose-950 rounded-2xl p-6 md:p-10 shadow-2xl relative overflow-hidden flex flex-col items-center">
                    ...
                    <h2 className="text-pink-300 font-bold uppercase tracking-widest text-xs mb-8 z-10 text-center">Your Estimated Due Date</h2>
                    ...
                    <div className="z-10 relative mb-10 w-full max-w-sm">
                        <div className="p-8 rounded-full aspect-square border-4 bg-pink-900/40 border-pink-400/30 shadow-inner flex flex-col items-center justify-center">
                            <span className="text-white/60 text-xs uppercase tracking-widest mb-2 font-bold block text-center">Baby Arrives Around</span>
                            <div className="font-bold text-3xl md:text-4xl text-white text-center drop-shadow-lg leading-tight">
                                {result.dueDate.toLocaleDateString('en-US', { month: 'long', day: 'numeric' })}<br />
                                <span className="text-pink-300 text-2xl">{result.dueDate.getFullYear()}</span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6 w-full max-w-4xl z-10 mb-8">
                        <div className="bg-black/30 p-5 rounded-xl border border-white/10 flex flex-col items-center justify-center text-center">
                            <span className="text-pink-400 text-[10px] font-bold uppercase tracking-widest mb-2">Current Gestational Age</span>
                            <div className="font-mono text-white text-2xl font-bold">
                                {result.gestationalAge.weeks} <span className="text-sm text-pink-300 tracking-normal font-sans">wks</span> {result.gestationalAge.days} <span className="text-sm text-pink-300 tracking-normal font-sans">days</span>
                            </div>
                        </div>
                        <div className="bg-black/30 p-5 rounded-xl border border-white/10 flex flex-col items-center justify-center text-center">
                            <span className="text-pink-400 text-[10px] font-bold uppercase tracking-widest mb-2">Current Trimester</span>
                            <div className="font-mono text-white text-2xl font-bold">
                                {result.trimester}{result.trimester === 1 ? 'st' : result.trimester === 2 ? 'nd' : 'rd'} <span className="text-sm text-pink-300 tracking-normal font-sans">Trimester</span>
                            </div>
                        </div>
                        <div className="bg-black/30 p-5 rounded-xl border border-white/10 flex flex-col items-center justify-center text-center">
                            <span className="text-pink-400 text-[10px] font-bold uppercase tracking-widest mb-2">Days Remaining</span>
                            <div className="font-mono text-white text-2xl font-bold">
                                {result.daysRemaining} <span className="text-sm text-pink-300 tracking-normal font-sans">Days</span>
                            </div>
                        </div>
                    </div>

                    <div className="w-full max-w-4xl z-10 bg-black/40 p-6 rounded-2xl border border-pink-500/20">
                        <h3 className="text-pink-300 font-bold uppercase tracking-widest text-xs mb-6 text-center border-b border-pink-500/20 pb-3">Pregnancy Timeline Milestones</h3>

                        <div className="space-y-4">
                            <div className="flex justify-between items-center text-sm">
                                <span className="text-white/80 font-medium tracking-wide">Estimated Conception</span>
                                <span className="text-white font-bold bg-white/10 px-3 py-1 rounded-full">{formatDate(result.conceptionDate)}</span>
                            </div>
                            <div className="flex justify-between items-center text-sm">
                                <span className="text-white/80 font-medium tracking-wide">End of 1st Trimester (Week 13)</span>
                                <span className="text-white font-bold bg-white/10 px-3 py-1 rounded-full">{formatDate(result.firstTrimesterEnd)}</span>
                            </div>
                            <div className="flex justify-between items-center text-sm">
                                <span className="text-white/80 font-medium tracking-wide">End of 2nd Trimester (Week 27)</span>
                                <span className="text-white font-bold bg-white/10 px-3 py-1 rounded-full">{formatDate(result.secondTrimesterEnd)}</span>
                            </div>
                            <div className="flex justify-between items-center text-sm pb-1">
                                <span className="text-pink-300 font-bold tracking-wide">Estimated Due Date (Week 40)</span>
                                <span className="text-pink-100 font-bold bg-pink-600/50 px-3 py-1 rounded-full border border-pink-500">{formatDate(result.dueDate)}</span>
                            </div>
                        </div>
                    </div>
                </div>
