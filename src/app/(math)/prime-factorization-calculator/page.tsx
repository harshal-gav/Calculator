...
export default function PrimeFactorizationCalculator() {
  ...
  const [factors, setFactors] = useState<
    { prime: number; power: number }[] | null
  >(null);
  ...
  const calculate = () => {
    ...
    if (num > Number.MAX_SAFE_INTEGER || num > 999999999999) {
      setIsError(true);
      setErrorMsg(
        "Number is too large or outside safe integer bounds for client-side processing.",
      );
      setFactors(null);
      return;
    }
  ...
  return (
    <div className="max-w-3xl mx-auto p-4 md:p-8 bg-zinc-50 rounded-2xl shadow-xl border border-zinc-200">
      ...
      <div className="bg-white p-6 md:p-10 rounded-2xl shadow-sm border border-zinc-200">
        <label className="block text-sm font-bold text-zinc-500 mb-2 uppercase tracking-widest text-center">
          Enter a Number
        </label>
      ...
      {factors && (
        <div className="mt-8 bg-emerald-950 rounded-2xl p-6 md:p-12 shadow-2xl relative overflow-hidden text-center border border-emerald-800">
          ...
          <div className="relative z-10 w-full flex flex-col items-center">
            <h2 className="text-emerald-400 font-bold uppercase tracking-widest text-sm mb-6">
              Prime Factors
            </h2>
            ...
            {factors.length === 1 && factors[0].power === 1 ? (
              <div className="bg-zinc-800/80 backdrop-blur border border-emerald-500/50 p-6 rounded-2xl">
                <span className="text-3xl text-emerald-300 font-bold block mb-2">
              ...
              <div className="flex flex-wrap items-center justify-center gap-3 text-4xl md:text-5xl font-mono font-bold text-white tracking-widest bg-black/40 p-8 rounded-2xl border border-white/10 shadow-inner break-all">
                {factors.map((f, i) => (
                  <div key={f.prime} className="flex items-center">
                    ...
                    {f.power > 1 && (
                      <sup className="text-2xl md:text-3xl ml-1 text-emerald-200">
                        {f.power}
                      </sup>
                    )}
                    {i < factors.length - 1 && (
            ...
            {factors.length > 0 &&
              !(factors.length === 1 && factors[0].power === 1) && (
                <div className="mt-8 pt-6 border-t border-emerald-800 w-full max-w-lg">
                  <div className="text-emerald-600 text-xs font-bold uppercase tracking-widest mb-3">
                    Expanded Form
                  </div>
                  <div className="text-xl md:text-2xl font-mono text-emerald-200/80 break-all leading-loose">
                    {factors
                      .map((f) => Array(f.power).fill(f.prime).join(" × "))
                      .join(" × ")}
                  </div>
                </div>
              )}
      ...
      <div className="mt-8">
        <CalculatorSEO
          ...
          whatIsIt={
            <>
              <p>
                A <strong>Prime Factorization Calculator</strong> is an
                analytical math tool designed to break down any composite
                integer into the fundamental prime numbers that, when multiplied
                together, produce the original number.
              </p>
              <p>
                Every integer greater than 1 is either a standalone prime or can
                be decomposed into a unique set of prime factors. Our tool
                automatically displays both the standard multiplied visual
                representation (e.g., 2 × 2 × 3) and the elegant algebraic
                exponential form.
              </p>

              <p className="mt-4 text-sm text-gray-500">
                <strong>Related Terms:</strong> Prime Factorization Calculator
              </p>
            </>
          }
          formula={
          ...
          example={
            <>
              <p>
                Let’s determine the prime factorization of <strong>84</strong>{" "}
              ...
              <ol className="list-decimal pl-6 space-y-2 mt-4 font-mono text-sm bg-black/5 p-4 rounded-xl border border-black/10">
                <li>
                  Divide by 2 (the smallest prime): 84 ÷ 2 = <strong>42</strong>
                </li>
                <li>
                  Divide by 2 again: 42 ÷ 2 = <strong>21</strong>
                </li>
                <li>
                  21 isn't divisible by 2. Next smallest prime is 3: 21 ÷ 3 ={" "}
                  <strong>7</strong>
                </li>
          ...
          useCases={
            <ul className="list-disc pl-6 space-y-4">
              <li>
                <strong>Finding GCF and LCM:</strong> Prime factorization is the
                foolproof underpinning method used to manually calculate the
                Greatest Common Factor and Least Common Multiple of multiple
                numbers.
              </li>
              <li>
                <strong>Simplifying Fractions:</strong> Decomposing large
                numerators and denominators into primes makes canceling shared
                factors trivial, allowing you to instantly reduce fractions to
                their lowest terms.
              </li>
              <li>
                <strong>Cryptography:</strong> Prime numbers and factorization
                form the absolute mathematical foundation for RSA encryption
                algorithms that secure modern internet communications.
              </li>
            </ul>
          }
          faqs={[
            {
              ...
              answer:
                "No, the number 1 is specifically excluded from being prime because a prime number must have exactly two distinct positive divisors: 1 and itself. Furthermore, including 1 would violate the Fundamental Theorem of Arithmetic regarding unique factorization.",
            },
            {
              question:
                "What happens if I put a prime number into the calculator?",
              answer:
