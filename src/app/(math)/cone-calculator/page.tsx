        ...
        <div className="max-w-4xl mx-auto p-4 md:p-8 bg-white rounded-xl shadow-lg border border-gray-100">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-8">
                ...
                <div className="bg-white border-2 border-dashed border-emerald-200 rounded-xl p-6 flex items-center justify-center min-h-[250px] relative overflow-hidden">
                    ...
                    <svg viewBox="0 0 100 100" className="w-48 h-48 drop-shadow-xl text-emerald-600 z-10" fill="none" stroke="currentColor" strokeWidth="2">
                        {/* Base ellipse */}
            ...
            {isValid ? (
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                    <div className="bg-emerald-600 text-white p-5 rounded-xl shadow-md transform transition hover:-translate-y-1">
                        <div className="text-xs font-bold uppercase tracking-wider mb-1 text-emerald-200">Volume</div>
                        <div className="text-2xl font-black">{volume.toFixed(2)}</div>
                        <div className="text-xs mt-1 text-emerald-200 font-mono">V = ⅓πr²h</div>
                    </div>
                    <div className="bg-emerald-700 text-white p-5 rounded-xl shadow-md transform transition hover:-translate-y-1">
                        <div className="text-xs font-bold uppercase tracking-wider mb-1 text-emerald-200">Total Surface Area</div>
                        <div className="text-2xl font-black">{surfaceArea.toFixed(2)}</div>
                        <div className="text-xs mt-1 text-emerald-200 font-mono">A = πr(r + s)</div>
                    </div>
                    <div className="bg-emerald-800 text-white p-5 rounded-xl shadow-md transform transition hover:-translate-y-1">
                        <div className="text-xs font-bold uppercase tracking-wider mb-1 text-emerald-200">Lateral Surface Area</div>
                        <div className="text-2xl font-black">{lateralArea.toFixed(2)}</div>
                        <div className="text-xs mt-1 text-emerald-200 font-mono">L = πrs</div>
                    </div>
                    <div className="bg-emerald-900 text-white p-5 rounded-xl shadow-md transform transition hover:-translate-y-1">
                        <div className="text-xs font-bold uppercase tracking-wider mb-1 text-emerald-200">Slant Height (s)</div>
                        <div className="text-2xl font-black">{slantHeight.toFixed(2)}</div>
                        <div className="text-xs mt-1 text-emerald-200 font-mono">s = √(r² + h²)</div>
                    </div>
                </div>
            ) : (
