    ...
    <div className="max-w-4xl mx-auto p-4 md:p-8 bg-zinc-50 rounded-2xl shadow-xl border border-zinc-200">
      <div className="text-center mb-10">
        ...
        <p className="text-emerald-700 text-lg max-w-2xl mx-auto">
          Calculate the Nth term and the sum of the first N terms for Arithmetic
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) => setCommonDiffRatio(e.target.value)}
              className="w-full rounded-xl border-zinc-300 shadow-sm p-4 border focus:border-emerald-500 font-bold font-mono text-lg transition-all"
            />
          </div>
          <div>
        ...
        <div className="bg-emerald-950 rounded-2xl p-6 md:p-10 shadow-2xl relative overflow-hidden flex flex-col items-center">
          ...
          <div className="w-full max-w-3xl z-10 grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
            <div className="bg-black/40 border border-emerald-500/30 p-8 rounded-3xl shadow-inner text-center">
              ...
              <div className="font-mono font-black text-4xl md:text-5xl text-white break-all tracking-tight drop-shadow-lg">
                {Number.isInteger(result.nthTerm)
                  ? result.nthTerm
                  : result.nthTerm.toLocaleString("en-US", {
            ...
            <div className="bg-black/40 border border-emerald-500/30 p-8 rounded-3xl shadow-inner text-center">
              ...
              <div className="font-mono font-black text-4xl md:text-5xl text-white break-all tracking-tight drop-shadow-lg">
                {Number.isInteger(result.sumToN)
                  ? result.sumToN
                  : result.sumToN.toLocaleString("en-US", {
                      maximumFractionDigits: 4,
                    })}
              </div>
            </div>
          ...
          <div className="w-full max-w-2xl z-10 space-y-4">
            <div className="bg-emerald-900/40 p-5 rounded-2xl border border-emerald-500/20 text-center">
              <span className="text-emerald-300 text-[10px] font-bold uppercase tracking-widest mb-2 block">
                Explicit Formula
              </span>
            ...
            <div className="bg-emerald-900/40 p-5 rounded-2xl border border-emerald-500/20 text-center">
              ...
              <div className="flex flex-wrap justify-center gap-2">
                {result.sequencePoints.map((val, idx) => (
                  <span
                    key={idx}
                    className="bg-emerald-950/80 text-white font-mono text-sm px-3 py-1 rounded border border-emerald-700/50"
                  >
                    {Number.isInteger(val) ? val : Number(val.toPrecision(4))}
                  </span>
                ))}
                {parseInt(termN) > 10 && (
                  <span className="text-emerald-500 px-2 py-1">...</span>
                )}
              </div>
            </div>
          </div>
      ...
      <div className="mt-8">
        <CalculatorSEO
          ...
          whatIsIt={
            <>
              <p>
                The <strong>Sequence Calculator</strong> mathematically projects
                and sums standard numerical progressions. It supports the two
                fundamental types of mathematical sequences: Arithmetic and
                Geometric.
              </p>
              <p>
                An Arithmetic Sequence changes by consistently{" "}
                <em>adding or subtracting</em> the same value. A Geometric
                Sequence changes by consistently{" "}
                <em>multiplying or dividing</em> by the same value.
              </p>

              <p className="mt-4 text-sm text-gray-500">
                <strong>Related Terms:</strong> Sequence Calculator
              </p>
            </>
          }
          formula={
          ...
          example={
            <>
              ...
              <p>
                You have a starting value of <strong>2</strong> and a common
                difference of <strong>+3</strong>. You want to find the 5th term
                in the sequence.
              </p>
              <ul className="list-disc pl-6 space-y-1 mt-2 text-zinc-700 mb-4">
                <li>
                  <strong>The Sequence:</strong> 2, 5, 8, 11, 14.
                </li>
                <li>
                  The 5th Term (a₅) is exactly <strong>14</strong>.
                </li>
                <li>
                  The Sum of those 5 terms (S₅) is 2+5+8+11+14 ={" "}
                  <strong>40</strong>.
                </li>
              </ul>
            </>
          ...
          useCases={
            <ul className="list-disc pl-6 space-y-4 text-zinc-700">
              <li>
                <strong>Compound Interest (Geometric):</strong> Calculating how
                an initial investment grows when multiplying by 1.05 (a 5%
                return) every year for 30 years.
              </li>
              <li>
                <strong>Depreciation Schedules (Arithmetic):</strong> Modeling
                the straight-line book value of heavy machinery losing exactly
                $1,500 of value every single month.
              </li>
              <li>
                <strong>Computer Algorithm Analysis:</strong> Using geometric
                sum formulas to calculate the O(N) memory complexity and
                branching limits of deeply recursive data structures like Binary
                Trees.
              </li>
            </ul>
          }
